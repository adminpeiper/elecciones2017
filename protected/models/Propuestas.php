...
class Propuestas extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('idcandidato, idcategoria, descripcion, estado', 'required'),
			array('idcandidato, idcategoria', 'numerical', 'integerOnly'=>true),
			array('descripcion', 'length', 'max'=>500),
			array('estado', 'length', 'max'=>1),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('idpropuestas, idcandidato, idcategoria, descripcion, estado', 'safe', 'on'=>'search'),
		);
	}
	...
	public function search()
	{
		...
		$criteria->compare('idpropuestas',$this->idpropuestas);
